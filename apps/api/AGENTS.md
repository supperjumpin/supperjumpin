...
| Task | Location | Notes |
|------|----------|-------|
| Entry point / wiring | `cmd/api/main.go` | Env vars: PORT, DATABASE_URL, SUPABASE_JWT_SECRET, SUPPERJUMPIN_DEV_AUTH_TOKEN. npm scripts use `SUPPERJUMPIN_DATABASE_URL` to opt into non-local DBs. |
...
- **Auth middleware pattern**: Every protected route calls `signedInProfile(w, r, config)` first. Bearer token from `Authorization` header. Production/staging auth verifies Supabase HS256 JWTs with `SUPABASE_JWT_SECRET`; local dev can also use `SUPPERJUMPIN_DEV_AUTH_TOKEN`.
