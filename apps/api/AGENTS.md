...
| Task | Location | Notes |
|------|----------|-------|
| Entry point / wiring | `cmd/api/main.go` | Env vars: PORT, SUPPERJUMPIN_DATABASE_URL, SUPPERJUMPIN_DEV_AUTH_TOKEN. npm scripts use local Docker Postgres by default and pass `SUPPERJUMPIN_DATABASE_URL` explicitly. |
