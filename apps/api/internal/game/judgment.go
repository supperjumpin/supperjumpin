...
type JudgmentRepository interface {
	...
	UpsertJudgment(ctx context.Context, jumpID, playerID string, commitment, transgression, creativity, presentation int) (Judgment, bool, error)
...
func SubmitJudgment(ctx context.Context, repo JudgmentRepository, input JudgmentInput, now time.Time) JudgmentResult {
	...
	// 4. Author Grace Period must have expired.
	if !now.After(jump.GracePeriodExpiresAt) {
		return JudgmentResult{Err: ErrAuthorGracePeriodActive}
	}

	// 5. Judge must not be the performer. Group membership is NOT required.
	if jump.PlayerID == input.JudgePlayerID {
		return JudgmentResult{Allowed: false}
	}

	// 6. Check judging window for season-linked jumps
	...
	// 7. Persist the judgment (upsert)
	judgment, created, err := repo.UpsertJudgment(ctx, input.JumpID, input.JudgePlayerID, input.Commitment, input.Transgression, input.Creativity, input.Presentation)
	if err != nil {
		return JudgmentResult{Err: err}
	}

	// 8. On first valid judgment, transition the Jump to "Judged Jump"
	if created {
