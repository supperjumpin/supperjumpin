# apps/bot-discord KNOWLEDGE BASE
...
| Bot core (commands, dispatcher) | `internal/bot/` | Pure bot — no `discordgo` import. Handlers per command, bot dispatch by `CommandRoute` (slash) or `CustomID` prefix (component) |
