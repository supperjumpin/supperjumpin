...
The backend follows a split strategy: narrow unit tests for domain logic, Postgres-backed tests for transport and lifecycle coverage, and no shared in-memory test double as the default path. Mobile starts with manual verification plus typecheck, while `api-client` only needs tests around the hand-written runtime wrapper, not the generated types.
