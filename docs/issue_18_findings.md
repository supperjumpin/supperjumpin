...
- **Authorization**: The `AuthorizeEvidenceUpload` endpoint strictly requires a stunt to be in `Planned Stunt` status and verifies the performer's identity.
- **Verification & Transition**: The `SubmitEvidence` endpoint verifies the upload authorization, checks that the season's submission window is currently open, and atomically transitions the stunt status from `Planned Stunt` to `Performed Stunt`.
- **Data Model**: The `evidences` and `evidence_upload_authorizations` tables are correctly structured to maintain the 1:1 relationship between evidence and stunts.
...
- **User Experience**: The app effectively stops at the "Planned Stunt" stage.
