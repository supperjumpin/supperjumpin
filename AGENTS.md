...
npm run db:migrate        # Apply migrations to DATABASE_URL using repo-local golang-migrate
...
- `DATABASE_URL` is mandatory — API refuses to start without it. Use local Docker Postgres for normal dev; use Supabase staging only intentionally. See `docs/supabase.md`.
