...
usage() {
  ...
  printf '\n'
  printf 'Runs a bounded OpenCode Ralph AFK loop targeting one GitHub issue PRD.\n'
  printf '\n'
  printf 'Arguments:\n'
...
4. If there is no remaining work, or the next work needs a human decision, add a short PRD comment and output <promise>COMPLETE</promise>.
...
PROMPT
  ...
  if [[ "$result" == *'<promise>COMPLETE</promise>'* ]]; then
    printf 'PRD complete or blocked; exiting.\n'
    exit 0
  fi
done
